...
build:
	...
	$(CONTAINER_RUNTIME) run -e CARTO_API_KEY=$(CARTO_API_KEY) -e OPENAIP_API_KEY=$(OPENAIP_API_KEY) --rm -v $(shell pwd)/$(INPUT_DIR):/data/$(INPUT_DIR) -v $(shell pwd)/$(OUTPUT_DIR):/data/$(OUTPUT_DIR) -v $(CACHE_DIR):/root/.cache/kml-heatmap $(IMAGE_NAME) $(INPUT_DIR) --output-dir $(OUTPUT_DIR)
