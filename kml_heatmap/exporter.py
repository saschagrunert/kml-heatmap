    ...
    def calculate_rolling_average_speeds(self):
        ...
        for i, seg in enumerate(self.segment_speeds):
            ...
            if current_timestamp is not None and timestamp_list:
                ...
                window_distance = sum(time_indexed_segments[j]['distance'] for j in range(start_idx, end_idx))
                window_time = sum(time_indexed_segments[j]['time_delta'] for j in range(start_idx, end_idx))
...
def calculate_path_duration_and_distance(
    ...
    if start_ts and end_ts:
        try:
            ...
            logger.debug(f"Could not parse path timestamps '{start_ts}' -> '{end_ts}': {e}")
...
def export_airports_json(
    ...
    with open(airports_file, 'w') as f:
        json.dump({'airports': valid_airports}, f, separators=(',', ':'), sort_keys=True)
...
def export_metadata_json(
    ...
    meta_data = {
        ...
        'min_alt_m': min_alt_m,
        'max_alt_m': max_alt_m,
        'min_groundspeed_knots': round(speed_stats['min'], 1) if speed_stats['min'] != float('inf') else 0,
